...
void Cutscene::init(sf::Font& t_font)
{
	...
	{
		std::cout << "Error with cutscene player texture";
	}

	m_playerSprite.setTexture(m_playerTexture);
	m_playerSprite.setScale(1.2, 1.2);
	m_playerSprite.setOrigin(m_playerSprite.getGlobalBounds().width / 2, m_playerSprite.getGlobalBounds().height / 2);
	m_playerSprite.setPosition(ScreenSize::s_width - m_playerSprite.getGlobalBounds().width * 1.5, 900);
